...
Live playback automatically routes Twitch and other recognized URLs through Streamlink, including with buffering enabled. Finite video URLs stream on demand; MPV shows the full duration and supports seeking anywhere. Unrendered sections need a short render wait. Explicit Download first still downloads the source. Live channels remain forward-only. Both paths use temporary SDR encoded video. Buffered DLSS processes frames independently to avoid motion warping from estimated temporal guides. The GPU/model stays loaded; MPV display smoothing and HDR remain available. Normal live playback keeps its existing temporal path. A sustained rate below 1× will eventually require refilling; a larger buffer cannot cure that indefinitely. Fit player window uses source dimensions; other output choices set render bounds before playback. Temporary capture stops around 4 GB or when less than 1 GB disk space remains. Stop removes the session's temporary files.
...
Live RTX Processing offers Off, VSR, HDR, or both. **100% full quality** remains the default and retains MPV scaling before DLSS. The optional **VSR first · 1080p cap** work-size mode runs source → VSR cleanup (up to 1920×1080) → DLSS Super Resolution plus neural enhancement → optional HDR. Sources above the 1920×1080 bounds skip VSR based on their original dimensions; smaller windows do not change that decision. A 1080p source receives a same-size VSR cleanup request. Aspect ratio is preserved, and VSR is not run a second time after DLSS. Small windows may downscale the input to fit. HDR requires Windows HDR enabled. The log distinguishes pre-DLSS VSR from post-DLSS HDR and reports actual neural dimensions. Driver acceptance of a VSR request does not by itself confirm enhancement; NVIDIA's indicator remains the activation check. The mode uses an SDR intermediate, so native HDR content is tone-mapped before neural enhancement and HDR expansion.
