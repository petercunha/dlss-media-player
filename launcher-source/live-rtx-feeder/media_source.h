...
static bool Update(UINT bw,UINT bh){
 ...
 static ULONGLONG next=0;static int w=0,h=0,l=0,t=0,cw=0,ch=0;
 if(GetTickCount64()>=next){
  next=GetTickCount64()+100;
  FILE* f=_wfopen(Path(),L"r");int a,b,c,d,e,g;
  if(f){if(fscanf(f,"%d %d %d %d %d %d",&a,&b,&c,&d,&e,&g)==6){w=a;h=b;l=c;t=d;cw=e;ch=g;}else w=0;fclose(f);}else w=0;
 }
 if(w!=(int)bw||h!=(int)bh||l<0||t<0||cw<2||ch<2||cw>w||ch>h||l>w-cw||t>h-ch)return false;
 x=l;y=t;width=cw&~1u;height=ch&~1u;active=true;return true;
}
