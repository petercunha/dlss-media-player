...
class LiveRtxTests{
 ...
 static async Task Test(string[] a){
  var e=new Engine(a[0]);e.LiveRtxMode=2;e.LiveTarget=int.Parse(a[2]);e.Log=Console.WriteLine;e.SetWorkResolution(100);
  ...
  if(log.Contains("VSR requested=1"))throw new Exception("VSR must remain disabled");
  Console.WriteLine("PASS: DLSS -> HDR live GPU pipeline and shutdown");
 }
}
